...
#[tokio::test]
async fn http_error_body_read_failure_is_preserved() {
    ...
    let client = TenantApiClient::new(&format!("http://{addr}"), "default").unwrap();

    let err = client
        .post_json::<serde_json::Value, serde_json::Value>("/fail-read", &serde_json::json!({}))
        .await
        .unwrap_err();

    match err {
        ClientError::HttpStatusBodyRead { status, .. } => assert_eq!(status, 400),
        other => panic!("expected HttpStatusBodyRead, got {other:?}"),
    }
}
