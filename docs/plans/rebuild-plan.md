...
This section describes the intended repo layout at a high level. It is not a literal dump of every directory on disk. The target shape below focuses on the tracked product and workspace structure.
...
├── CLAUDE.md                     # Repo-specific contributor/agent guidance
├── config/
│   ├── app.toml                  # Non-secret configuration
│   ├── agents/                   # Agent YAML specs (post-MVP)
│   └── prompts/                  # Prompt templates (file-based for MVP)
├── data/
│   ├── demo/                     # Sample corpora for manual smoke testing
│   ├── docker/                   # Local Postgres/Qdrant dev volumes and init files
│   └── test/                     # Tracked test data and fixtures
...
│   │   ├── src/
│   │   └── tests/                # CLI end-to-end tests
│   ├── agent-core/               # Graph-flow agent orchestration (post-MVP)
│   │   ├── src/
│   │   └── tests/                # Runtime/spec integration tests
...
│       └── src/
├── docs/
│   └── howto/                    # Tracked developer documentation
├── schemas/                      # JSON/OpenAPI/schema artifacts
├── .github/
│   └── workflows/
│       └── ci.yml
└── rustfmt.toml
...
- [x] Integration test: modify a file → chunks updated in place (stable UUID overwrite); explicit old-chunk cleanup path is not applicable in the current design
...
**Status: COMPLETE** ✓ — Stale chunk cleanup is a no-op (stable UUIDs overwrite in place); modify-file integration test exists for the supported overwrite path.
...
- [x] Smoke test: X-Tenant isolation through API
...
**Status: COMPLETE** ✓ — Shipped more endpoints than planned. End-to-end API journey and X-Tenant API isolation smoke tests exist.
...
- [x] End-to-end test: start server → CLI ingest → CLI chat → verify response
...
**Status: COMPLETE** ✓ — Shipped search subcommand, interactive chat, and an ignored CLI ingest→chat end-to-end subprocess test beyond plan.
...
- Vertical-slice task registry (classify, hybrid search, summarize, approval checkpoint, final answer)
- Query classification (Structured/Unstructured/Mixed)
- Checkpoint approval/rejection
- Timeline event logging
- Run lifecycle management inside `agent-core`
...
- **Matched (or improved):** checkpoint approval/rejection runtime behavior

**Key deliverables:**
- [x] Agent graph builder from YAML spec
- [ ] All search tool implementations
- [ ] ReAct loop with configurable stop conditions
- [x] Checkpoint lifecycle
- [x] Timeline event collection
...
**Status: PARTIALLY COMPLETE** — `agent-core` now includes YAML spec loading and validation, a graph-flow-backed DAG runtime, checkpoint pause/resume lifecycle, and step/timeline capture for the vertical-slice spike. Search tool coverage, ReAct execution, and server/API integration remain outstanding.
...
- 5 RBAC roles (Viewer, Editor, AgentOperator, Admin, PlatformOperator) with 7 capabilities
- Capability-based authorization (not role comparison) — fail-closed on unknown routes
- Rate limiting (global RPS + burst + concurrency, per-tenant/principal/route-class)
- API key lifecycle (generate, revoke, list) with SHA-256 hashing at rest
- OIDC platform group → PlatformOperator promotion

**Simplified vs matched:**
- **Matched (or improved):** auth header semantics (Bearer + X-Api-Key), role hierarchy, rate limit behavior
- **Improved:** API-key principal binds to DB tenant (not request header) — prevents cross-tenant escalation
- **Improved:** OIDC role lookup fails closed on DB errors and invalid role strings (not default-to-Viewer)
- **Improved:** Rate limit 429 responses include `Retry-After: 1` header
- **Deferred:** JWKS cache TTL tuning, clock skew leeway (hardening)
- **Deferred:** `/license/status` endpoint (hardening phase)

**Key deliverables:**
- [x] Auth middleware with pluggable mode (none/api-key/oidc)
- [x] API key validation (format: `apex_<prefix>_<secret>`, SHA-256 hashed)
- [x] OIDC JWT validation (RS256/384/512 via JWKS cache)
- [x] RBAC enforcement middleware (capability-based, fail-closed)
- [x] Rate limiting middleware (token bucket, global + per-tenant)
- [x] `Principal` identity type unifying all auth modes
- [x] Migration `0003_auth.sql` (service_accounts, api_keys, oidc_principals)
- [x] API key management endpoints (create SA, generate key, revoke, list)
- [x] CLI `api-key generate` subcommand
- [x] Middleware layer ordering: request_id → tenant → auth → rate_limit → authz → handler
- [x] Integration tests for auth modes (3 tests, ignored — requires `just up`)
- [x] Unit tests: roles/capabilities, token bucket, route classification, principal identity
- [x] Cross-tenant escalation prevention (API-key principal bound to DB tenant)
- [x] OIDC platform group members promoted to PlatformOperator role
- [x] Fail-closed OIDC: invalid role strings → 500, not default Viewer

**Dependencies:** Phase 7 (server middleware stack).

**Risks:**
- ~~OIDC testing requires a local Keycloak or similar.~~ Unit tests validate JWT structure; full OIDC flow requires Keycloak (deferred to integration environment).
- ~~Rate limiter state management (in-memory vs Redis).~~ In-memory token bucket with DashMap. Redis backend deferred — DashMap entries are never evicted (documented as a gap for long-running servers).

**Exit criteria:** All three auth modes work. RBAC correctly restricts endpoints. Rate limiting enforced.

**Status: COMPLETE** ✓ — PR #14. Capability-based RBAC, 3 auth modes, rate limiting with Retry-After headers. 4 rounds of security review resolved (cross-tenant fix, fail-closed OIDC, platform group promotion).
...
**Behaviors reimplemented:**
- PDF, Markdown, and plain-text extraction
...
- [x] OCR with Tesseract subprocess management
...
**Status: PARTIALLY COMPLETE** — PDF/MD/TXT extractors are in place, and Tesseract subprocess-managed OCR fallback for scanned PDFs already shipped. The remaining parity extractors and archive/email support are still outstanding.
